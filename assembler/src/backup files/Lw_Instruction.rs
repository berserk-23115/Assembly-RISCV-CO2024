...
fn lw_type(input_str: &str) // taking input string as parameter , output file header
{
    ...
    let operands: Vec<&str> = instruction[1].split(",").collect(); //SEPERATES {Return Address Register(ABI)} AND {Source Register1(ABI)}{,}{Immediate[11:0}  seperated
    ...
    println!(
        "{} {} {} {} {}",
        &immediate_bin, registers[&source_reg], "010", register_bin, "0000011"
    );
}
fn main() {
    let st: &str = "lw a5,20(s1)";
    lw_type(st);
}
